namespace YanZhiwei.DotNet3._5.Utilities.Common
{
    ...
    public static class TypeHelper
    {
        ...
        public static bool AttributeExists<T>(this MemberInfo memberInfo, bool inherit = false)
            where T : Attribute
        {
        ...
        public static bool IsGenericAssignableFrom(this Type genericType, Type type)
        {
            ...
            List<Type> _allOthers = new List<Type> { type };
            if (genericType.IsInterface)
            {
                _allOthers.AddRange(type.GetInterfaces());
            }

            foreach (var other in _allOthers)
            {
                Type _cur = other;
                while (_cur != null)
                {
                    if (_cur.IsGenericType)
                    {
                        _cur = _cur.GetGenericTypeDefinition();
                    }
                    if (_cur.IsSubclassOf(genericType) || _cur == genericType)
                    {
                        return true;
                    }
                    _cur = _cur.BaseType;
