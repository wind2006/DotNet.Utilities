...
namespace PetaPoco
{
    ...
    public class Database : IDisposable
    {
        ...
        public delegate void DataChangedHanlderEvent(Type type, string sql);

        /// <summary>
        /// 数据添加，删除，修改的事件
        /// </summary>
        public event DataChangedHanlderEvent DataChangedEvent;

        private void OnDataChangedEvent(Type type, string sql)
        {
            if (DataChangedEvent != null)
            {
                DataChangedEvent(type, sql);
            }
        }
        ...
        /// 时间：2016-03-28 9:39
        /// 备注：
        internal void OnExecutedCommand(IDbCommand cmd, OperateState operateState, Type type, int effectCount)
        {
            if ((effectCount > 0) || (operateState == OperateState.Added || operateState == OperateState.Deleted || operateState == OperateState.Modified))
            {
                OnDataChangedEvent(type, cmd.GetGeneratedQuery());
            }
        }

        internal void OnExecutedCommand(IDbCommand cmd, OperateState operateState, Type type, object returnValue)
        {
            if ((returnValue != null) && (operateState == OperateState.Added || operateState == OperateState.Deleted || operateState == OperateState.Modified))
            {
                OnDataChangedEvent(type, cmd.GetGeneratedQuery());
            }
        }
        ...
        internal int Execute(OperateState operateState, Type type, string sql, params object[] args)
        {
            try
            {
                OpenSharedConnection();
                try
                {
                    ...
                    {
                        var retv = cmd.ExecuteNonQuery();
                        OnExecutedCommand(cmd, operateState, type, retv);
                        return retv;
                    }
                }
                finally
        ...
        internal int Execute(OperateState operateState, Type type, Sql sql)
        {
            return Execute(operateState, type, sql.SQL, sql.Arguments);
        }
        ...
        //internal T ExecuteScalar<T>(OperateState operateState, string sql, params object[] args)
        //{
        //    try
        //    {
        //        OpenSharedConnection();
        ...
        //            {
        //                object val = cmd.ExecuteScalar();
        //                OnExecutedCommand(cmd, operateState, val);
        //                return (T)Convert.ChangeType(val, typeof(T));
        //            }
        //        }
        //        finally
        ...
        {
            return MultiPocoQuery<TRet>(new Type[] { typeof(T1), typeof(T2), typeof(T3) }, cb, sql, args);
        }

        public IEnumerable<TRet> Query<T1, T2, T3, T4, TRet>(Func<T1, T2, T3, T4, TRet> cb, string sql, params object[] args)
        {
            return MultiPocoQuery<TRet>(new Type[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4) }, cb, sql, args);
        }
        ...
        {
            return MultiPocoQuery<TRet>(new Type[] { typeof(T1), typeof(T2), typeof(T3) }, cb, sql.SQL, sql.Arguments);
        }

        public IEnumerable<TRet> Query<T1, T2, T3, T4, TRet>(Func<T1, T2, T3, T4, TRet> cb, Sql sql)
        {
            return MultiPocoQuery<TRet>(new Type[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4) }, cb, sql.SQL, sql.Arguments);
        }
        ...
        {
            return MultiPocoQuery<T1>(new Type[] { typeof(T1), typeof(T2), typeof(T3) }, null, sql, args);
        }

        public IEnumerable<T1> Query<T1, T2, T3, T4>(string sql, params object[] args)
        {
            return MultiPocoQuery<T1>(new Type[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4) }, null, sql, args);
        }
        ...
        {
            return MultiPocoQuery<T1>(new Type[] { typeof(T1), typeof(T2), typeof(T3) }, null, sql.SQL, sql.Arguments);
        }

        public IEnumerable<T1> Query<T1, T2, T3, T4>(Sql sql)
        {
            return MultiPocoQuery<T1>(new Type[] { typeof(T1), typeof(T2), typeof(T3), typeof(T4) }, null, sql.SQL, sql.Arguments);
        }
        ...
        private IEnumerable<TRet> MultiPocoQuery<TRet>(Type[] types, object cb, string sql, params object[] args)
        {
            OpenSharedConnection();
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        if (!autoIncrement)
                        {
                            DoPreExecute(cmd);
                            // cmd.ExecuteNonQuery();
                            OnExecutedCommand(cmd, OperateState.Added, poco.GetType(), cmd.ExecuteNonQuery());
                            return true;
                        }
                        ...
                        switch (_dbType)
                        {
                            case DBType.SqlServerCE:
                                DoPreExecute(cmd);
                                cmd.ExecuteNonQuery();
                                OnExecutedCommand(cmd, OperateState.Added, poco.GetType(), 1);
                            ...
                            case DBType.SqlServer:
                                ...
                                OnExecutedCommand(cmd, OperateState.Added, poco.GetType(), id);
                                break;
            ...
            try
            {
                ...
                try
                {
                    using (var cmd = CreateCommand(_sharedConnection, ""))
                    {
                        ...
                        // Do it
                        var retv = cmd.ExecuteNonQuery();
                        OnExecutedCommand(cmd, OperateState.Modified, poco.GetType(), retv);
                        return retv;
                    }
                }
                finally
        ...
        {
            var pd = PocoData.ForType(typeof(T));
            return Execute(OperateState.Modified, typeof(T), string.Format("UPDATE {0} {1}", EscapeTableName(pd.TableInfo.TableName), sql), args);

            //if (Execute(string.Format("UPDATE {0} {1}", EscapeTableName(pd.TableInfo.TableName), sql), args) > 0) {
            //    OnDataChangedEvent(cmd.CommandText);
            // }
        }
        ...
        {
            var pd = PocoData.ForType(typeof(T));
            return Execute(OperateState.Modified, typeof(T), new Sql(string.Format("UPDATE {0}", EscapeTableName(pd.TableInfo.TableName))).Append(sql));
        }
        ...
        public int Delete(string tableName, string primaryKeyName, object poco, object primaryKeyValue)
        {
            ...
            return Execute(OperateState.Deleted, poco.GetType(), sql, primaryKeyValue);
        }
        ...
        {
            var pd = PocoData.ForType(typeof(T));
            return Execute(OperateState.Deleted, typeof(T), string.Format("DELETE FROM {0} {1}", EscapeTableName(pd.TableInfo.TableName), sql), args);
        }

        public int Delete<T>(Sql sql)
        {
            var pd = PocoData.ForType(typeof(T));
            return Execute(OperateState.Deleted, typeof(T), new Sql(string.Format("DELETE FROM {0}", EscapeTableName(pd.TableInfo.TableName))).Append(sql));
        }
        ...
        internal class PocoData
        {
            ...
            private bool IsIntegralType(Type t)
            {
